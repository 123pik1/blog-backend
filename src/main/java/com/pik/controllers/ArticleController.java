...
public class ArticleController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ArticleDTO> editArticle(@RequestBody ArticleDTO article, @PathVariable Long id) {
        if (!id.equals(article.getId())) {
            return ResponseEntity.badRequest().build();
        }

        ArticleDTO response = articleService.updateArticle(article);
        if (response == null)
